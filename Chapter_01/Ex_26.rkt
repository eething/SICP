...
(define (expmod base exp m expmode)
  (cond ((= expmode 0)
        ...
        ((= expmode 1)
               (cond ((= exp 0) 1)
               ((even? exp) (remainder (* (expmod base (/ exp 2) m expmode)
                                          (expmod base (/ exp 2) m expmode))
                                       m))
               (else (remainder (* base (expmod base (- exp 1) m expmode)) m))))))
